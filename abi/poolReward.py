...
with open("./abi_json/uniswapRouterv2.json") as f:
    uniswapABI = json.loads(f.read())

with open("./abi_json/pool4.json") as f:
    poolABI = json.loads(f.read())

with open("./abi_json/vault.json") as f:
    vaultABI = json.loads(f.read())

with open("./abi_json/uniswap_token.json") as f:
...
def get_data(pool, rewardTokenAddress, reward_price, lp_price):
    ...
    return {"apy": apy, "totalStakedAmount": stake_lp, "TVL": tvl, 'WeeklyROI': WeeklyROI}
